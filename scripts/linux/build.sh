...
# the archive_engine modules it needs: arc_util, arc_net, arc_tag).
#
...
# enough for arc_net. core/tag has no such system fallback (TagLib ships no
...
# just accept that this script's smoke build stops at arc_tag; a real
# consumer (e.g. streamer) vendors its own TagLib and add_subdirectory()s
# this repo's core/ after it, at which point arc_tag links fine — see
